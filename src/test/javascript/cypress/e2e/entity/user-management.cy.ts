...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Ross_Lowe66', email: 'Ismael_Kling@hotmail.com' };
  ...
  describe.skip('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Friedrich57');
      cy.get(`[data-cy="login"]`).should('have.value', 'Friedrich57');

      cy.get(`[data-cy="firstName"]`).type('Julian');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Julian');

      cy.get(`[data-cy="lastName"]`).type('Haag');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Haag');

      cy.get(`[data-cy="email"]`).type('Helen.Wehner@yahoo.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Helen.Wehner@yahoo.com');
