  ...
  return (
    <div>
      <h2 id="post-comment-heading" data-cy="PostCommentHeading">
        <Translate contentKey="unipassWebApp.postComment.home.title">Post Comments</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" variant="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="unipassWebApp.postComment.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/post-comment/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="unipassWebApp.postComment.home.createLabel">Create new Post Comment</Translate>
          </Link>
        </div>
      </h2>
        ...
        {postCommentList?.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="unipassWebApp.postComment.id">ID</Translate>{' '}
                ...
                <th>
                  <Translate contentKey="unipassWebApp.postComment.post">Post</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="unipassWebApp.postComment.author">Author</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {postCommentList.map(postComment => (
                <tr key={`entity-${postComment.id}`} data-cy="entityTable">
                  <td>
                    <Button as={Link as any} to={`/post-comment/${postComment.id}`} variant="link" size="sm">
                      {postComment.id}
                    </Button>
                  </td>
                  <td>{postComment.content}</td>
                  <td>
                    {postComment.createdAt ? <TextFormat type="date" value={postComment.createdAt} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>
                    {postComment.updatedAt ? <TextFormat type="date" value={postComment.updatedAt} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>{postComment.post ? <Link to={`/community-post/${postComment.post.id}`}>{postComment.post.title}</Link> : ''}</td>
                  <td>{postComment.author ? postComment.author.login : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="unipassWebApp.postComment.home.notFound">No Post Comments found</Translate>
            </div>
          )
        )}
      </div>
