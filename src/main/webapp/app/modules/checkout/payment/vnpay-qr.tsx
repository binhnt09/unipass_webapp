import { ArrowLeft, CheckCircle, X, Clock, Copy, Check, CreditCard, QrCode, ShieldCheck, Building2 } from 'lucide-react';
import { Link } from 'react-router';
import React, { useState, useEffect } from 'react';

export function VNPayQRPage() {
  const [isPaid, setIsPaid] = useState(false);
  const [timeLeft, setTimeLeft] = useState(900); // 15 minutes in seconds
  const [copiedField, setCopiedField] = useState<string | null>(null);

  const orderDetails = {
    orderId: 'UM2024032302',
    amount: 5400000,
    itemName: 'Tai nghe Sony WH-1000XM4 + Đèn bàn LED',
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-cyan-50 to-indigo-100 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-3xl shadow-2xl border border-gray-200 overflow-hidden">
          {/* Header with VNPay Branding */}
          <div className="bg-gradient-to-br from-[#0B5DBB] via-[#1E88E5] to-[#0B5DBB] p-8 text-center relative overflow-hidden">
            {/* Decorative elements */}
            ...
            <div className="relative z-10">
              <div className="w-24 h-24 bg-white rounded-3xl flex items-center justify-center mx-auto mb-4 shadow-2xl">
                <Building2 className="w-12 h-12 text-[#0B5DBB]" />
              </div>
              <h2 className="text-3xl font-bold text-white mb-2">Thanh toán VNPay</h2>
              <p className="text-white/90 text-sm mb-4">Quét mã QR bằng ứng dụng ngân hàng của bạn</p>
          ...
          <div className="p-8">
            ...
            <div className="flex items-center justify-center gap-2 mb-8">
              <div className="flex items-center gap-3 bg-gradient-to-r from-blue-50 to-cyan-50 px-6 py-3 rounded-full border-2 border-blue-200">
                <div className="relative">
                  <div className="w-3 h-3 bg-blue-500 rounded-full animate-ping absolute"></div>
                  <div className="w-3 h-3 bg-blue-500 rounded-full"></div>
                </div>
                <span className="text-sm font-bold text-blue-700">Đang chờ thanh toán</span>
              </div>
            </div>

            {/* QR Code Section */}
            <div className="mb-8">
              <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-8 border-2 border-dashed border-gray-300 hover:border-[#0B5DBB] transition-colors">
                <div className="bg-white rounded-2xl p-6 shadow-xl">
                  {/* QR Code */}
                  <div className="aspect-square max-w-xs mx-auto bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl flex items-center justify-center p-8 border-4 border-[#0B5DBB]">
                  ...
                  <div className="text-center mt-4">
                    <div className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-100 to-cyan-100 px-4 py-2 rounded-full">
                      <QrCode className="w-4 h-4 text-[#0B5DBB]" />
                      <span className="text-sm font-bold text-[#0B5DBB]">Mã QR VNPay</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 space-y-3">
                <div className="flex items-start gap-3 bg-blue-50 rounded-lg p-4 border border-blue-200">
                  <div className="w-6 h-6 bg-[#0B5DBB] text-white rounded-full flex items-center justify-center flex-shrink-0 font-bold text-sm">
                    1
                  </div>
                  <p className="text-sm text-gray-700">
                    <strong className="text-[#0A2647]">Mở ứng dụng ngân hàng</strong> hoặc <strong className="text-[#0A2647]">VNPay</strong>{' '}
                    trên điện thoại
                  </p>
                </div>
                <div className="flex items-start gap-3 bg-blue-50 rounded-lg p-4 border border-blue-200">
                  ...
                  <p className="text-sm text-gray-700">
                    <strong className="text-[#0A2647]">Chọn {'Quét QR'}</strong> hoặc{' '}
                    <strong className="text-[#0A2647]">{'Thanh toán QR'}</strong>
                  </p>
                </div>
                <div className="flex items-start gap-3 bg-blue-50 rounded-lg p-4 border border-blue-200">
                  <div className="w-6 h-6 bg-[#0B5DBB] text-white rounded-full flex items-center justify-center flex-shrink-0 font-bold text-sm">
                  ...
                  <p className="text-sm text-gray-700">
                    <strong className="text-[#0A2647]">Quét mã QR</strong> bên trên và{' '}
                    <strong className="text-[#0A2647]">xác nhận thanh toán</strong>
                  </p>
                </div>
              </div>
            </div>
            ...
            {/* Supported Banks */}
            <div className="mb-8">
              <h4 className="text-sm font-bold text-gray-700 mb-3 text-center">Ngân hàng hỗ trợ</h4>
              <div className="flex flex-wrap justify-center gap-3">
                {['Vietcombank', 'BIDV', 'VietinBank', 'Agribank', 'Techcombank', 'MB Bank', 'ACB', 'TP Bank'].map(bank => (
                  <div
                    key={bank}
                    className="px-3 py-1.5 bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-200 rounded-lg text-xs font-medium text-blue-700"
                  >
                    {bank}
                  </div>
                ))}
            ...
            <div className="bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 rounded-2xl p-6 mb-6 border-2 border-indigo-200 shadow-md">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gradient-to-r from-[#0B5DBB] to-[#1E88E5] rounded-xl p-6 shadow-lg">
            ...
            <div className="flex items-center justify-center gap-2 mb-6">
              ...
              <span className="text-sm text-gray-600">Giao dịch được mã hóa và bảo mật bởi VNPay</span>
            </div>
            ...
            <div className="space-y-3">
              <button
                ...
                className="w-full py-4 bg-gradient-to-r from-[#0B5DBB] via-[#1E88E5] to-[#0B5DBB] hover:shadow-xl text-white rounded-xl font-bold text-lg transition-all shadow-lg flex items-center justify-center gap-2"
              >
              ...
              <Link
                to="/payment"
                className="w-full py-3.5 bg-white border-2 border-gray-300 hover:bg-gray-50 hover:border-[#0B5DBB] text-gray-700 rounded-xl font-medium transition-all flex items-center justify-center gap-2"
              >
                <X className="w-5 h-5" />
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in">
            <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 text-center animate-in">
              ...
              <p className="text-gray-600 mb-8 text-lg">
                Chúng tôi đang xác minh giao dịch của bạn qua VNPay. Bạn sẽ nhận được thông báo trong vài phút.
              </p>
              <div className="space-y-3">
                <Link
                  to="/purchase-success"
