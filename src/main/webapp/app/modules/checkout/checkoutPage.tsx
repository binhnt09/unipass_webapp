...
export function CheckoutPage() {
  ...
  const [addresses, setAddresses] = useState<ShippingAddress[]>([
    ...
    },
    {
      ...
      name: 'Minh Hoàng',
      phone: '0912 345 678',
      address: 'Thư viện Trung tâm, Đại học FPT',
      isDefault: false,
    },
  ]);
  ...
  useEffect(() => {
    // If no data from cart, redirect back
    ...
    const seller = allSellerGroups.find(g => g.sellerId === sellerId);
    if (!seller) {
      navigate('/cart');
      return;
    }
    ...
    const selectedItems = seller.items.filter(item => selectedItemIds.includes(item.id));
  ...
  }, [sellerId, selectedItemIds, allSellerGroups, navigate]);
  ...
  const shippingInfo = addresses.find(addr => addr.id === selectedAddressId) || addresses[0];
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div key={item.id} className="flex gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-lg font-bold text-[#FF5722]">${(item.price * item.quantity).toFixed(2)}</span>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="pt-4 border-t-2 border-gray-300">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-2xl font-bold text-[#FF5722]">${totalPayment.toFixed(2)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6">
                {!isAddingNewAddress ? (
                  ...
                  <>
                    ...
                    <h3 className="text-lg font-bold text-[#0A2647] mb-4">Thêm địa chỉ mới</h3>
        ...
        {/* NEW FEATURE: Order Preview Modal - review order details before final confirmation */}
                ...
                <div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-medium text-gray-900">{shippingInfo.phone}</span>
                    </div>
                    <div className="pt-2 border-t">
                      <span className="text-sm text-gray-600 block mb-1">Địa chỉ:</span>
                      <span className="font-medium text-gray-900">{shippingInfo.address}</span>
                    </div>
                    {checkoutGroup?.deliveryMethod && (
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between pt-2 border-t">
                        ...
                        <span className="text-lg font-bold text-[#FF6B35]">
                          ${(checkoutGroup?.items.reduce((sum, item) => sum + item.price * item.quantity, 0) || 0).toFixed(2)}
                        </span>
                      </div>
                    </div>
                  </div>
