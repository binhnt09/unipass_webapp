...
import { Edit, Trash2, Plus, Bell, X, BadgeCheck, Mail, Loader2, Pause, Play } from 'lucide-react';
...
export function SellerDashboardPage() {
  ...
  const handleAccept = async () => {
    ...
    try {
      await axios.patch(`/api/products/${selectedListing}`, { id: selectedListing, status: 'PENDING_DEAL' });
  ...
  const handleDecline = async () => {
    if (!selectedListing) return;
    try {
      await axios.patch(`/api/products/${selectedListing}`, { id: selectedListing, status: 'AVAILABLE' });
      // Update local state directly so color shift reflects instantly
